export default {
    settings: {
      ...
      payment: {
        ...
        providerWechatXpay: '微信虚拟支付(个人主体)',
        typeDisabled: '类型已禁用',
        ...
        field_appKey: '现网 AppKey',
        field_products: '面额 → 道具 ID 映射',
        field_env: '环境标识',
        field_platform: '客户端平台',
        field_offerIdHint: '微信 MP 后台 → 虚拟支付 → 基本配置 里的 OfferID。',
        field_appKeyHint: '虚拟支付基本配置里的「现网 AppKey」，服务端用它计算 paySig。',
        field_productsHint: 'JSON 映射：键是面额（单位「分」），值是「道具管理」里的道具 ID，例如 {"500":"道具ID-1"}。只有配了道具的档位才会出现在充值页。注意：道具单价由微信后台固定，所以本渠道的「充值费率」必须设为 0，否则金额对不上道具价格。',
        field_envHint: '固定填 0（正式环境/现网）。',
        field_platformHint: '可选。客户端平台标识；若微信返回 signData 校验失败，可尝试填 android 或 ios。',
        field_cid: '支付渠道 ID',
