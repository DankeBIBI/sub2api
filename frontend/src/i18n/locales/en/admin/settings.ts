export default {
    settings: {
      ...
      payment: {
        ...
        providerWechatXpay: 'WeChat Virtual Payment (personal)',
        ...
        field_offerId: 'OfferID',
        field_appKey: 'Production AppKey',
        field_products: 'Amount → product ID map',
        field_env: 'Environment',
        field_platform: 'Client platform',
        field_offerIdHint: 'OfferID from the WeChat MP console → Virtual Payment → Basic settings.',
        field_appKeyHint: 'The production AppKey from the same page; the server uses it to compute paySig.',
        field_productsHint: 'JSON map: the key is the amount in fen, the value is the product ID from the console product list, e.g. {"500":"product-id-1"}. Only mapped tiers appear on the recharge page. Note: product prices are fixed in the WeChat console, so the recharge fee rate for this channel must be 0.',
        field_envHint: 'Always 0 for production.',
        field_platformHint: 'Optional client platform flag; try android or ios if WeChat rejects signData.',
