...
type CreateOrderRequest struct {
	...
	PaymentType string
	OpenID      string
	// JsCode 与 SessionKey 仅微信虚拟支付使用:前者用于换 openid + session_key,
	// 后者用于签 signature。其它渠道留空。
	JsCode          string
	SessionKey      string
	ClientIP        string
	IsMobile        bool
...
type CreateOrderResponse struct {
	...
	// Xpay 仅在微信虚拟支付(result_type=xpay_ready)时返回。
	Xpay                          *payment.WechatXpayPayload `json:"xpay,omitempty"`
